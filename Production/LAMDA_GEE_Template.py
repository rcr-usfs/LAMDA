...
nDaysDict = {"CONUS": 16, "AK": 32, "AK_main": 32, "AK_SE": 32}
...
export_area_dict = {
    "AK": ee.FeatureCollection("projects/gtac-lamda/assets/Ancillary/AK_main").merge(ee.FeatureCollection("projects/gtac-lamda/assets/Ancillary/AK_se")),
...
lcms = ee.ImageCollection("USFS/GTAC/LCMS/v2023-9").filter(ee.Filter.calendarRange(startYear, endYear, "year"))
...
tcc = tcc.select(["NLCD_Percent_Tree_Canopy_Cover"]).max().gte(10).selfMask()
...
hansen = ee.Image("UMD/hansen/global_forest_change_2023_v1_11").select([0]).gte(5)
