...
def matrix_linregress(y,x = '', vertical_view = False, return_residuals = True, compute_r2 = True,x_term = 'x',in_no_data = '',out_no_data = ''):
    ...
    if in_no_data != '' and in_no_data != None:
        for i in range(len(out)):
            print(('Masking layer here', i + 1))
            out[i][msk] = out_no_data
        msk = None
...
def new_big_image_matrix_linregress(in_image,out_image, years = '', in_no_data = 255,out_no_data = -600, mem_size_limit = 20000, layer_names = ['slope', 'b', 'r2','RMSE'], band_list = [],dt = 'Float32',compression = True):
    ...
    if os.path.exists(out_image)== False:
        tb1 = time.time()
        ti= tiled_image(out_image,in_image,bands = 4,dt = dt, size_limit_kb = mem_size_limit, outline_tiles = True, out_no_data = out_no_data,compression = compression)
        i = 1
...
def aggregate_raster(in_raster,out_raster, factor = 2, ignore_no_data = True):
    ...
    chunk_xys = make_chunk_coords(ri['width']-factor,ri['height']-factor,5000)
    print((len(chunk_xys)))
    ti = tiled_image(out_raster,'',ri['width']/factor,ri['height']/factor,ri['bands'],ri['dt'],out_transform,ri['projection'],'HFA',True,50000,True,out_no_data = ri['no_data'])
    tile_i = 1
    for xo,yo,w,h in chunk_xys:
        print(('Aggregating tile', tile_i,'/',len(chunk_xys)))
        #Iterate through the bands
        for b in range(1,ri['bands']+1):

            #Read in raster of band n
            r = raster(in_raster,'',b,xo,yo,w+factor,h+factor)
            ...
            ti.add_tile(ot,xo/factor,yo/factor, b)
...
##############################################################
#Function for setting up square tiles of a specified size
def make_chunk_coords(width,height,tile_size = 500):
        ...
        for x in xs:
            for y in ys:
                if x + tile_size > width:
                    ...
                    tile_widthT = tile_size
                if y + tile_size > height:
                    tile_heightT = height-y
                else:
                    tile_heightT = tile_size
                xys.append([x,y,tile_widthT,tile_heightT])
...
###########################################################################################
    ...
    def chunk_pointer_maker(self, size_limit_kb = 100):
        ...
        break_points_x = list(range(0, int(self.width), int(chunk_size_x)))
        break_points_y = list(range(0, int(self.height), int(chunk_size_y)))
...
def initialize_raster(output_name,template = '', df = 'HFA', dt = 'Int16', width = '', height = '', bands = 1, projection = '', transform = '', out_no_data = '', compress = False):
    ...
    if out_no_data != '' and out_no_data != None:   
        ...
        # null_array = numpy.full((height,width),out_no_data) 
        for i in range(1,bands+1):

            ds.GetRasterBand(i).SetNoDataValue(out_no_data)
            ds.GetRasterBand(i).Fill(out_no_data)
        # null_array = None
    ds = None
#######################################################################################
...
def find_count(in_string, what_to_find):
    ...
    for i in range(len(in_string)):
        if in_string.find(what_to_find,i) ==i:
            fc += 1
    return fc
def smart_sort(in_list, year_list = list(range(1980, 2030)), year_query = 'base(il).find(str(year))'):
    out_list = []
    out_year_list = []
    t_lists = []
    duplicate_list = []
...
def simple_smart_sort(in_list, year_list = list(range(1980, 2030)), year_query = 'base(il).find(str(year))'):
    yo = []
    io = []
    for year in year_list:
        for il in in_list:
...
def zonal_stat(input_zone_polygon, input_value_raster, FID = 0, statistic = 'numpy.mean',band_list  = 'All', no_data_value = ''):
    if band_list.lower() == 'all':
        ...
        band_list = list(range(1,ii['bands']+1))
    if no_data_value == '':
        ri = raster_info(input_value_raster)
        no_data_value = ri['no_data']
    shp = ogr.Open(input_zone_polygon)
...
def focal_filter(Input, Output = '', fun = 'mean', matrix = 'low_pass', kernel_size = 3, return_array = True):
    ...
    while int(kernel_size)%2 == 0:
        ...
        kernel_size = int(input('Please re-enter the kernel size: '))
    if type(matrix) == str:
        matrix_type = string.lower(matrix)
        matrix = []
        numbers = matrix_dict[matrix_type]
        mid = middle(list(range(kernel_size)))
...
def break_strata(strata_raster = '', out_dir = '', dt = '', exclude_list = [],overwrite = False, guiable = True):
    ...
    strata = list(range(numpy.amin(array), numpy.amax(array) + 1, 1))
    out_list = []
    for num in strata:
...
def variety(input_list, output = '', dt = 'Int16', hist_only = False):
    ...
    bins = list(range(0, len(input_list) + 1))
...
def hist(image_list = [], table_name = '', bins ='', dt = '', band_no = 1, guiable = True):
    ...
    for image in image_list:
        ...
        if bins == '':
            bins = list(range(numpy.amin(array), numpy.amax(array) + 2, 1))
    ...
    columns = list(range(0,len(hist_list.split('\n')[0].split('\t')) -1, 1))
    header = 'Image_Name'
...
def unstack(stk = '', out_dir = '',dt = '', layers = 'All', overwrite = False, guiable = True,out_name = ''):
    if stk == '':
        stk = str(askopenfilename(title = 'Select image to unstack',filetypes=[("IMAGINE","*.img"),("tif","*.tif")]))
    if dt == '':
        info = raster_info(stk)
        dt = info['dt']
    rast = gdal.Open(stk)
    ...
    layer_list = []
    for i2 in layers:
        print('Unstacking layer',i2)


        if out_dir == '':
            out_name = os.path.splitext(stk)[0] + '_' + str(int(i2)) + '.img'
        else:
            out_name = out_dir + os.path.basename(os.path.splitext(stk)[0]) + '_' + str(int(i2)) + '.img'
        print(out_name,i2)
        if os.path.exists(out_name) == False or overwrite == True:
            array = raster(stk, band_no = i2, dt = dt)
##            print i,array

            write_raster(array, out_name, stk, dt = dt, bands = 1)
...
def limitProcesses(processLimit):
  while len(multiprocessing.process.active_children()) > processLimit:
    ...
    time.sleep(5)
def new_unstack(stk, out_dir,start_band_suffix = 1):
  ri = raster_info(stk)
  # src_ds = gdal.Open(stk)
  for b in range(1,ri['bands']+1):
    band_suffix = str(start_band_suffix)
    out_name = out_dir + base(stk).split('_Stack')[0] + '_' + band_suffix + '.tif'
    if not os.path.exists(out_name):
      print(('Unstacking: '+base(out_name)))
      
      translateoptions = gdal.TranslateOptions(gdal.ParseCommandLine("-of Gtiff -co COMPRESS=LZW -b "+ str(b)))
      out_ds = gdal.Translate(out_name, stk, options = translateoptions)
      out_ds=None  
    start_band_suffix +=1
def batch_unstack(stacks,out_dir,start_band_suffix):
  for s in stacks:
    p = Process(target = new_unstack,args = (s, out_dir,start_band_suffix,))
    p.start()
    limitProcesses(7)
  limitProcesses(0)
######################################################################################
...
def batch_convert(in_folder, out_folder, convert_from_extension = '.hdr', out_format = 'GTiff', overwrite = False):
    ...
    images = [i for i in os.listdir(in_folder) if os.path.splitext(i)[1] == convert_from_extension]
    images = [in_folder + i for i in images]
...
def gdal_sieve(Input, Output = None, threshold = 8, connectedness = 8,out_no_data = ''):
    ...
    if Output != None:
        ...
        Format = format_dict[os.path.splitext(Input)[1]]
...
def xy_coords(shapefile, write_csv = False, csv_name = 'default_csv.csv', addIndex = False,idField = ''):
    shapeData = ogr.Open(shapefile)
    layer = shapeData.GetLayer()
    points = []

    if idField != '' and idField != None:
        ids = multiple_field_extraction(shapefile,['plotid'])
    ...
    for index in range(layer.GetFeatureCount()):
        ...
        if addIndex:
            if idField != '' and idField != None:
                points.append([ids[index][0],geometry.GetX(), geometry.GetY()])
            else:
...
class fancy_merge_rasters:
    def __init__(self,merge_list, output_name, na_value = '', dt = '',overlap_function = 'numpy.amax', x_buffer = 50, y_buffer = 50, out_extent = [],compression = False,bands = None):
        self.tilei = 1
...
##############################################################################################
def merge_rasters(merge_list, output_name, na_value = '', overlap_function = 'numpy.amax', x_buffer = 50, y_buffer = 50, out_extent = [],compression = False,bands = None):
    if out_extent == [] or out_extent == None or out_extent == '':
        print('Finding union extent of images')
        extents = []
        i = 1
...
def sample_no_getter():
    ...
    try:
        ...
        input('Must restart program.  Press enter to exit')
        sys.exit()
    print(sample_no)
...
##    print 'Creating', output.split('/')[-1]
    if snap_raster != '':
        ...
        prj = web_mercator_crs
        projection = web_mercator_crs
        proj4 = web_mercator_crs
    if os.path.exists(output):
        delete_shapefile(output)
    #Project it
    ...
    if addXY:
        update_field(output, 'XS', xs, datatype = 'Real',fieldWidth =25)
        update_field(output, 'YS', ys, datatype = 'Real',fieldWidth =25)
...
def degreeTileMaker(output,size = 10,proj4 = '+proj=longlat +datum=NAD83 +no_defs'):
    ...
    xs = list(range(-180,171,size))
    ys = list(range(-90,81,size))[::-1]
...
def big_stratified_sampler(image, sample_nos, sample_name, dt = '', wo_replacement = False, shuffle_out = True):
    ...
    class_codes = list(sample_nos.keys())
    class_codes.sort()
    ...
    for class_code in class_codes:
        ...
        if l > 0:
            ...
            if wo_replacement:
                ...
                st = random.sample(list(range(l)), sample_no)
            else:
        ...
        try:
            ...
            for i in range(len(sample_no)):
                ...
                if wo_replacement == False:
                    ...
                    random_numbers = (random.sample(list(range(len(table[i]))), int(sample_no[i])))
...
def simple_circle(size):
    xs_temp = list(range(0,6350, 25))
    ys_temp = list(range(0, 6350, 25))
...
def quad_coords(coordinates, size_x = 0.125, size_y = 0.125, seedx = 0.0, seedy = 0.0):
    ...
    xs = list(range(seedx, int(math.ceil(coordinates[2])), size_x))
    ys = list(range(seedy, int(abs(math.ceil(coordinates[1]))) + size_y, size_y))
...
def which_quad(coords, x_size = 0.125, y_size = 0.125):
    ...
    poss_x = list(range(int(0 + (0.5 * x_size)), int(1000 + (0.5 * x_size)), int(x_size)))
    poss_y = list(range(int(0 + (0.5 * y_size)), int(1000 + (0.5 * y_size)), int(y_size)))
